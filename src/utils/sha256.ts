// sha256.ts — Deterministic SHA-256 hash, works in any context.
//
// Uses crypto.subtle when available (fast, native).
// Falls back to pure JS implementation on non-secure contexts (HTTP proxy, Burp).
...
export async function sha256(input: string): Promise<string> {
  ...
  if (globalThis.crypto?.subtle) {
    ...
    return hexFrom(new Uint8Array(hash));
  }

  return hexFrom(sha256js(data));
}
